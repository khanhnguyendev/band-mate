...
export default async function ListeningPage() {
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: '2rem' }}>
      ...
      <h1 style={{ marginTop: '1.5rem' }}>Listening Practice</h1>
      <p style={{ color: '#6b7280', marginTop: 0 }}>
        Listen to the audio and answer the questions. Scoring is instant — no credits required.
      </p>
      ...
      <div style={{ marginTop: '1.5rem', display: 'grid', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
              <div>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                  <span style={{ background: '#e0f2fe', color: '#0369a1', fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: 999, fontWeight: 600 }}>
                    🎧 Audio
                  </span>
              ...
              <Link
                href={`/listening/${s.id}`}
                style={{ padding: '0.6rem 1.2rem', background: '#0369a1', color: '#fff', borderRadius: 8, textDecoration: 'none', fontWeight: 600, fontSize: '0.9rem', whiteSpace: 'nowrap', flexShrink: 0 }}
              >
                Start →
              </Link>
            </div>
        ...
        {sets.length === 0 && (
          <p style={{ color: '#9ca3af' }}>No listening sets available yet.</p>
        )}
      </div>
    </main>
  )
