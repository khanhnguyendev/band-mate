...
@Processor(SCORE_SPEAKING_QUEUE, { concurrency: 2 })
export class ScoreSpeakingWorker extends WorkerHost {
  private readonly logger = new Logger(ScoreSpeakingWorker.name)
  ...
  async process(job: Job<ScoringJobData>): Promise<void> {
    ...
    const promptPack = await this.prisma.promptPack.findFirstOrThrow({
      where: { skill: 'speaking', isActive: true },
    })
    ...
    const userPrompt = (promptPack.userPromptTemplate as string)
      ...
      .replace('{{transcript}}', submission.transcript ?? '')
  ...
  async onFailed(job: Job<ScoringJobData>, error: Error): Promise<void> {
    ...
    this.logger.error(`Speaking scoring failed for ${submissionId}: ${error.message}`)
